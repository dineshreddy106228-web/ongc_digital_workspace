...
OUTPUT = Path("output/pdf/QC Portfolio Weekly Management Report.pdf")
...
def page_number(canvas, document):
    ...
    canvas.drawString(14 * mm, 10 * mm, "ONGC Laboratories - Corporate Chemistry | QC Portfolio Weekly Management Report")
...
def main():
    ...
    story = [
        Paragraph("QC Portfolio Weekly Management Report", title),
        Paragraph(
            f"Corporate Chemistry | Data validity: <b>{validity}</b> | "
            f"Latest weekly uploads from {reporting_labs} {'laboratory' if reporting_labs == 1 else 'laboratories'} | "
    ...
    story += [kpi_table, Spacer(1, 6), Paragraph("Week-on-Week Comparison", h2)]
    ...
    if comparison["available"]:
        ...
        story += [Paragraph("No prior weekly upload is available for a like-for-like comparison. This section will populate automatically after a laboratory's next weekly workbook is imported.", small), Spacer(1, 6)]
    ...
    if comparison["available"]:
        story += [Paragraph("Laboratory Week-on-Week Movement", h2)]
    ...
    story += [make_table(completed, [35*mm, 110*mm, 25*mm, 23*mm, 28*mm, 28*mm], header, body), PageBreak(), Paragraph("Weekly Chemical Performance Analytics", h2)]
