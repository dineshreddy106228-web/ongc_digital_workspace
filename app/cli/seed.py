...
@click.command("fix-password-hashes")
@click.option(
    "--password",
    default="TempPass@2026",
    show_default=True,
    help="Temporary password to set for all affected users.",
)
@click.option(
    "--username",
    default=None,
    help="Reset a single user only (by username). Omit to reset all users.",
)
@with_appcontext
def fix_password_hashes(password, username):
    """Re-hash all user passwords using pbkdf2:sha256.

    Werkzeug 3.x switched the default hashing algorithm to scrypt, which is
    unavailable on Python 3.9/macOS with some OpenSSL builds.  Running this
    command replaces every stored hash with a fresh pbkdf2:sha256 hash and
    forces a password-change prompt on next login.

    Usage examples:
        flask fix-password-hashes
        flask fix-password-hashes --password "MyNewTemp@99"
        flask fix-password-hashes --username superadmin
    """
    if username:
    ...
    click.echo(
        ...
        f"→ temporary password: {password!r}\n"
    )
    ...
    for user in users:
        user.set_password(password)
        user.must_change_password = True
        click.echo(f"  ✓  {user.username:<30}  (must change password on next login)")
        updated += 1
    ...
    try:
        ...
        db.session.rollback()
        raise click.ClickException(
            f"Database error while saving password hashes: {exc.__class__.__name__}"
        ) from exc
