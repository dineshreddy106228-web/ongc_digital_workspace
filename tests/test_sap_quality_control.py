...
def test_other_sap_reporting_lab_accepts_its_single_plant_snapshot(sap_app):
    ...
    batch = import_sap_lab_exports(
        "rgl_vadodara", _inspection_export(plant="11A1"), "SAP_INSPECTION_20260826.xlsx",
        _notification_export(plant="11A1"), "SAP_NOTIFICATIONS_20260826.xlsx", None,
    )
    db.session.commit()

    assert batch.lab_code == "rgl_vadodara"
    assert batch.plant_code == "11A1"
...
def test_sap_management_presentation_uses_the_current_snapshot(sap_app):
    from app.core.services.qc_presentation import build_sap_panvel_presentation
