...
Entry point:
    build_word_document(draft, sections, parameters, flags) -> bytes
    build_master_spec_document(all_specs, include_draft_note=False) -> bytes
"""
...
from csc_repository import get_impact_grade
...
def _ensure_logo() -> Path | None:
    """Resolve ONGC logo from repo assets (prefer JPEG), fallback to embedded."""
...
def build_master_spec_document(
    ...
    Each item in all_specs must have keys:
        draft      – dict from repo.load_draft()
        sections   – list from repo.load_sections()
        parameters – list from repo.load_parameters()
        flags      – list from repo.load_issue_flags()
...
def _configure_spec_header(doc: Document) -> None:
...
def _build_change_log_page(
    ...
    for idx, entry in enumerate(changelog, start=1):
        ...
        try:
            from csc_utils import fmt_ist
            date_str = fmt_ist(raw_dt).split(" ")[0] if raw_dt else "—"
        except Exception:
